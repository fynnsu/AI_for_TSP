...
class InstanceGenerator:
    ...
    def __init__(self, n_instances, n_nodes, w=(20, 40, 60, 80, 100), ylim=(0, 50), xlim=(0, 200), prize='distance',
                 seed=None, data_dir='data/valid', offset=0, verbose=False):
    ...
    def save_optw(self, i, inst, adj, x_dir='instances', adj_dir='adjs'):
        ...
        inst.to_csv(os.path.join(path_x, f'instance{self.offset + i + 1:07}.csv'), index=False, sep=',')
        adj.to_csv(os.path.join(path_adj, f'adj-instance{self.offset + i + 1:07}.csv'), index=False, sep=',')
